...
exports.getMarketSummary = (req, res) => {
	unirest
		.get(
			"https://yahoo-finance-low-latency.p.rapidapi.com/v6/finance/quote/marketSummary"
		)
		.query({
			region: region,
			lang: lang,
		...
		.end(result => {
			...
			db.collection("api")
				.doc("market")
				.set(result.body.marketSummaryResponse)
				.then(() => {
...
exports.getQuotes = (req, res) => {
	...
	unirest
		...
		.end(result => {
			...
			db.collection("api")
				.doc("quotes")
				.set(result.body.quoteResponse, { merge: true })
				.then(() => {
					return res.status(200).json({ success: "Success" });
				})
				.catch(error => {
...
exports.getOneTicker = (req, res) => {
	...
	unirest
		...
		.end(result => {
			...
			tickerObj = {
				dataGranularity:
					result.body.chart.result[0].meta.dataGranularity,
				...
				indicators: {
					...
					close:
						result.body.chart.result[0].indicators.quote[0].close,
			...
			db.collection("tickers")
				.doc(result.body.chart.result[0].meta.symbol)
				.set(tickerObj)
				.then(() => {
					return res.status(201).json({
						success: `Intraday values of ${ticker} with an interval of 1 minute are saved to the database`,
					});
				})
				.catch(error => {
					console.error(error);
		...
		.end(async result => {
			...
			tickerObj =
				// object for the first ticker
				{
					...
					indicators: {
						open:
							result.body.chart.result[0].indicators.quote[0]
								.open,
						close:
							result.body.chart.result[0].indicators.quote[0]
								.close,
						high:
							result.body.chart.result[0].indicators.quote[0]
								.high,
						low:
							result.body.chart.result[0].indicators.quote[0].low,
					},
				};
			batch.set(
				db
					...
					.doc(result.body.chart.result[0].meta.symbol),
				tickerObj
			); // commit of the first ticker

			for (
				let i = 0;
				i < result.body.chart.result[0].comparisons.length;
				i++
			) {
				tickerObj =
					// objects for the rest of tickers
					{
						dataGranularity:
							result.body.chart.result[0].meta.dataGranularity,
						range: result.body.chart.result[0].meta.range,
						...
						indicators: {
							open:
								result.body.chart.result[0].comparisons[i].open,
							close:
								result.body.chart.result[0].comparisons[i]
									.close,
							high:
								result.body.chart.result[0].comparisons[i].high,
				...
				batch.set(
					db
						.collection("tickers")
						.doc(result.body.chart.result[0].comparisons[i].symbol),
					tickerObj
